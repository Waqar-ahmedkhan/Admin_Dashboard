...
const UserListPage: React.FC = () => {
  ...
  const parseEntry = (entry: string): Date => {
    ...
    const [hour, minute] = timePart.split(":").map(Number); // Use const for minute
    let hourAdjusted = hour; // Rename to avoid reassigning hour directly
  ...
  const sortedUsers = [...filteredUsers].sort((a, b) => {
    let va: string | number | boolean | Date = a[sortField]; // Add Date to the union type
    let vb: string | number | boolean | Date = b[sortField]; // Add Date to the union type

    if (sortField === "entry") {
      va = parseEntry(va as string); // Returns Date, now compatible
      vb = parseEntry(vb as string); // Returns Date, now compatible
